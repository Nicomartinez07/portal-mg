...
test('test Editar un usuario', async ({ page }) => {
  ...
  await page.getByRole('row', { name: 'Carlos Martinez cmartinez@' }).getByRole('button').first().click();
  ...
  await expect(page.getByRole('cell', { name: 'Carlos Martinezz' })).toBeVisible();
});