// components/awss3/MultipleMediaUploadField.tsx
'use client';
...
type FileWithPreview = {
  ...
  preview: string;
  isVideo: boolean;
  isNew: boolean; // ✅ Para distinguir nuevas de actuales
};
...
type MultipleMediaUploadFieldProps = {
  ...
  maxFiles?: number;
  required?: boolean;
  currentUrls?: string[]; // ✅ Nueva prop: URLs de medios actuales
};
...
export default function MultipleMediaUploadField({
  ...
  maxFiles = 10,
  required = false,
  currentUrls = [], // ✅ Medios actuales desde la BD
}: MultipleMediaUploadFieldProps) {
  const [filesWithPreviews, setFilesWithPreviews] = useState<FileWithPreview[]>([]);
  const [isProcessing, setIsProcessing] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  // ✅ Cargar previews de medios actuales al montar
  useEffect(() => {
    if (currentUrls.length > 0 && value.length === 0) {
      const currentPreviews: FileWithPreview[] = currentUrls.map((url) => ({
        file: null as any, // No hay archivo porque es URL
        preview: url,
        isVideo: url.includes('.mp4') || url.includes('.mov') || url.includes('.webm'),
        isNew: false, // ✅ No es nueva
      }));
      setFilesWithPreviews(currentPreviews);
    }
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const currentCount = filesWithPreviews.filter(f => f.isNew).length;
    if (currentCount + selectedFiles.length > maxFiles) {
      alert(`Solo puedes subir un máximo de ${maxFiles} archivos nuevos`);
      return;
    }
  ...
  const handleRemove = (index: number) => {
    const itemToRemove = filesWithPreviews[index];
    
    if (itemToRemove.isNew) {
      // Si es nueva, remover del estado
      const newFilesWithPreviews = filesWithPreviews.filter((_, i) => i !== index);
      const newFilesIndex = filesWithPreviews.slice(0, index).filter(f => f.isNew).length;
      const newFiles = value.filter((_, i) => i !== newFilesIndex);
      
      setFilesWithPreviews(newFilesWithPreviews);
      onChange(newFiles);
    } else {
      // Si es actual, solo remover de la vista (no se puede eliminar las actuales)
      const newFilesWithPreviews = filesWithPreviews.filter((_, i) => i !== index);
      setFilesWithPreviews(newFilesWithPreviews);
    }
  };

  const newFilesCount = filesWithPreviews.filter(f => f.isNew).length;
      ...
      <label className="block text-sm font-medium text-gray-700">
        ...
        {filesWithPreviews.length > 0 && (
          <span className="ml-2 text-gray-500">
            ({newFilesCount} nuevas / {maxFiles} máx)
          </span>
        )}
      </label>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-4">
          {filesWithPreviews.map((item, index) => (
            <div key={index} className="relative group">
              ...
              {/* Botón para abrir en nueva pestaña si es actual */}
              {!item.isNew && (
                <a
                  href={item.preview}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="absolute top-2 left-2 bg-blue-500 text-white rounded-full p-1 hover:bg-blue-600 transition-colors shadow-lg"
                >
          ...
          >
            <div className="flex flex-col items-center justify-center pt-5 pb-6">
              ...
              <p className="mb-2 text-sm text-gray-500">
                <span className="font-semibold">Agregar fotos/videos</span>
              </p>
