// components/orders/modals/EditObservedPreAuthorizationModal.tsx
import React, { useEffect, useState } from "react";
import { FaPlus, FaTrash } from "react-icons/fa";
import { updateObservedPreAuthorization } from "@/app/(dashboard)/ordenes/insert/preAutorizacion/actions";
...
interface EditObservedPreAuthorizationModalProps {
  order: Order;
  onClose: () => void;
  onOrderUpdated: () => void;
}
...
export default function EditObservedPreAuthorizationModal({
  order,
  onClose,
  onOrderUpdated,
}: EditObservedPreAuthorizationModalProps) {
  const { user } = useUser();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (licensePlatePhoto || vinPlatePhoto || odometerPhoto) {
        ...
        const uploadResult = await uploadOrderPhotos(order.id, {
          licensePlatePhoto,
          vinPlatePhoto,
          odometerPhoto,
          additionalPhotos,
      ...
      const result = await updateObservedPreAuthorization(
        order.id,
        dataToSubmit,
        user.userId
      );
      ...
      if (result.success) {
        alert("✅ Orden actualizada y reenviada correctamente");
        onOrderUpdated();
        onClose();
      } else {
      ...
      <div className="bg-white rounded-lg w-[900px] max-h-[90vh] overflow-hidden flex flex-col shadow-lg text-sm">
        ...
        <div className="bg-white px-6 py-4 border-b border-gray-200 flex justify-between items-center sticky top-0 z-10">
          <h2 className="text-lg font-semibold">
            Editar Pre-Autorización Observada
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                ...
                disabled={isSubmitting}
              >
                {isSubmitting ? "Enviando..." : "Reenviar Pre-Autorización"}
              </button>
            </div>
          </form>
        </div>
